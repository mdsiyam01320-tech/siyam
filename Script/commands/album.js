...
module.exports.run = async function ({ api, event, args }) {
  ...
  let queryMap = {
    cartoon: "addVideo", photo: "addPhoto", lofi: "addLofi", sad: "addSad",
    funny: "addFunny", islamic: "addIslamic", horny: "addHorny", anime: "addAnime",
    love: "addLove", lyrics: "addLyrics", flower: "addFlower", sigma: "addSigma",
    aesthetic: "addAesthetic", cat: "addCat", ff: "addFf", sex: "addSex",
    football: "addFootball", girl: "addGirl", friend: "addFriend", cricket: "addCricket",
  };
  ...
  try {
    ...
    const saveRes = await axios.get(`${await baseApiUrl()}/album?add=${queryType}&url=${fileUrl}`);

    api.sendMessage(
      `───────────────\n» ✅ 𝗦𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆 𝗔𝗱𝗱𝗲𝗱\n» 🔰 𝗦𝘁𝗮𝘁𝘂𝘀: ${saveRes.data.data}\n» 🔥 𝗨𝗥𝗟: ${fileUrl}\n───────────────\n» 👤 𝆠፝𝐒𝐈𝐘𝐀𝐌-𝐇𝐀𝐒𝐀𝐍`,
      id,
      event.messageID
    );
  } catch (error) {
...
module.exports.handleReply = async function ({ api, event, handleReply }) {
  ...
  if (isNaN(reply) || reply < 1 || reply > 20) {
    return api.sendMessage(
      "───────────────\n» ⚠️ 𝗣𝗹𝗲𝗮𝘀𝗲 𝗿𝗲𝗽𝗹𝘆 𝘄𝗶𝘁𝗵 𝗮 𝗻𝘂𝗺𝗯𝗲𝗿 𝗯𝗲𝘁𝘄𝗲𝗲𝗻 𝟭 𝗮𝗻𝗱 𝟮𝟬.\n───────────────\n» 👤 𝆠፝𝐒𝐈𝐘𝐀𝐌-𝐇𝐀𝐒𝐀𝐍",
      id,
      event.messageID
    );
  }
  ...
  // Dynamic lock check via config permission instead of raw ID
  if ((reply === 7 || reply === 15) && !global.config.ADMINBOT.includes(event.senderID)) {
    return api.sendMessage(
      "───────────────\n» 🔒 𝗢𝗻𝗹𝘆 𝗕𝗼𝘁 𝗔𝗱𝗺𝗶𝗻𝘀 𝗰𝗮𝗻 𝗮𝗰𝗰𝗲𝘀𝘀 𝟭𝟴+ 𝗠𝗲𝗱𝗶𝗮!\n───────────────\n» 👤 𝆠፝𝐒𝐈𝐘𝐀𝐌-𝐇𝐀𝐒𝐀𝐍",
      id,
      event.messageID
    );
  }
  ...
  try {
    ...
    api.sendMessage(
      { body: formattedBody, attachment: fs.createReadStream(filename) },
      id,
      () => {
